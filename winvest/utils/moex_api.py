...
urls = {
    "history": "https://iss.moex.com/iss/history/engines/%(engine)s/markets/%(market)s/securities/%(security)s.json?from=%(from)s&till=%(till)s",
    ...
    def auth(self, user: str, password: str) -> HTTPStatus:
        response = self.session.get(
            ...
            headers={
                ...
                % base64.b64encode((user + ":" + password).encode("utf-8"))[:-1]
            },
        )
